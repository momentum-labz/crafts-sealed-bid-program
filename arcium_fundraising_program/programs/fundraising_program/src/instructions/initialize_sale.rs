...
use crate::state::{Sale, SaleStatus, STATE_LEN, MAX_BUCKET_COUNT};
...
/// Minimum commitment window: 1 minute for testing, 15 minutes for production
pub const MIN_COMMITMENT_WINDOW_SECONDS: i64 = 1 * 60;
...
pub fn handler_initialize_sale(
    ...
    require!(bucket_count >= 1 && bucket_count <= MAX_BUCKET_COUNT as u16, ErrorCode::InvalidBucketCount);
    require!(fdv_max - fdv_min >= bucket_count as u64, ErrorCode::InvalidBucketCount);
    ...
    require!(commitment_start > now, ErrorCode::InvalidTimestamps);
    require!(commitment_end > now, ErrorCode::InvalidTimestamps);
    ...
    sale.bucket_count = bucket_count;
    ...
    sale.marginal_fill_rate = None;
    sale.clearing_bucket_low = None;
    sale.clearing_bucket_high = None;
    sale.total_raised = None;
    sale.total_users = 0;
    ...
    sale.bids_checked = 0;
    sale.arcium_settled = false;

    // Initialize encrypted state to zeros (will be populated by init_auction_state MPC callback)
    sale.encrypted_state = [[0u8; 32]; STATE_LEN];
    sale.state_nonce = 0;
