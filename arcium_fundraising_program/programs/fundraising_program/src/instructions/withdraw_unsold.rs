...
pub struct WithdrawUnsold<'info> {
    ...
    #[account(address = sale.token_mint @ ErrorCode::InvalidParameters)]
    pub token_mint: Account<'info, Mint>,

    #[account(mut, address = sale.token_vault @ ErrorCode::InvalidParameters)]
...
pub fn handler_withdraw_unsold(ctx: Context<WithdrawUnsold>) -> Result<()> {
    ...
    let tokens_owed_to_users = if sale.status == SaleStatus::Cancelled || sale.status == SaleStatus::Refunding {
        ...
        if let (Some(clearing_fdv), Some(total_raised)) = (sale.clearing_fdv, sale.total_raised) {
            if clearing_fdv > 0 {
                let tokens_128 = (total_raised as u128)
    ...
    msg!(
        "Unsold tokens withdrawn: sale={}, amount={}, authority={}",
        sale.key(),
        remaining_tokens,
