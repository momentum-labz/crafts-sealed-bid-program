...
async function encryptBid(maxFdv: number, round: number): Promise<Buffer> {
  console.log(
    `  Encrypting max_fdv=$${(maxFdv / 1e6).toFixed(0)} for round ${round}...`,
  );
...
async function surfnetTimeTravel(
  connection: anchor.web3.Connection,
...
describe("V0.2-Drand Sealed-Bid Auction", () => {
  ...
  const program = anchor.workspace
    .FundraisingProgram as Program<FundraisingProgram>;
  ...
  it("Should initialize sale with drand configuration", async () => {
    ...
    console.log(
      `✓ Sale initialized with drand reveal round: ${drandRevealRound}`,
    );
  ...
  it("Should allow users to commit with encrypted bids", async () => {
    ...
    if (IS_SURFPOOL) {
      ...
      if (ct < commitmentStart) {
        console.log(
          `  Current time: ${ct}, window opens: ${commitmentStart}`,
        );
    ...
    console.log(
      `✓ Commitment window is now open (current time: ${currentTime})`,
    );
    ...
    for (let i = 0; i < testBids.length; i++) {
      ...
      console.log(
        `[User ${i}] ✓ Committed $${bid.amount.toLocaleString()} with encrypted bid`,
      );
  ...
  it("Should batch reveal encrypted bids", async () => {
    ...
    for (let i = 0; i < testBids.length; i++) {
      ...
      console.log(
        `[User ${i}] ✓ Decrypted: $${(decryptedMaxFdv / 1e6).toFixed(0)}`,
      );
    }
    ...
    for (let i = 0; i < testBids.length; i++) {
      ...
      console.log(
        `  User ${i}: $${(bid.maxFdvPlaintext.toNumber() / 1e6).toFixed(
          0,
        )} FDV`,
      );
    }
  ...
  it("Should propose settlement", async () => {
    ...
    console.log(
      `Proposing settlement at clearing FDV: $${(
        CLEARING_FDV.toNumber() / 1e6
      ).toFixed(0)}`,
    );
