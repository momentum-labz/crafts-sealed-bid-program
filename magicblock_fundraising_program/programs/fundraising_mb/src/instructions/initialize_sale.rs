...
/// Minimum commitment window: 15 minutes (1 min for testing)
...
pub fn handler_initialize_sale(
    ...
    // MagicBlock PER fields
    sale.per_validator = per_validator;
    sale.settlement_computed = false;
    sale.bids_zeroed = false;

    msg!("Sale initialized: {:?}", sale.key());
    msg!("PER validator: {}", per_validator);
