...
describe('V0.2-Drand Sealed-Bid Auction', () => {
  ...
  it('Should allow user to update bid price', async () => {
    ...
    const tx = await program.methods
      .updateBid(newEncryptedBid, new BN(drandRevealRound), Array.from(newHashCommitment) as any)
      .accounts({
        sale: salePda,
        sealedBid: sealedBidPda,
        user: user.publicKey,
  ...
  it('Should reject withdraw_raised to non-treasury address', async () => {
    ...
    try {
      ...
      assert.ok(
        err.message.includes('ConstraintAddress') ||
          err.message.includes('InvalidParameters') ||
          err.error?.errorCode?.code === 'ConstraintAddress',
